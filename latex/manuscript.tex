...
\begin{abstract} 
    Natural seepage is a significant contributor to marine hydrocarbon inputs. Remote and intermittent seeps are difficult to monitor \hl{in the field}, yet oil slicks can be observed from spaceborne 
    \hl{synthetic aperture radar (SAR)} \hl{due to differences in their backscatter},
    creating potential for automatic mapping. \hl{In mapping tasks such as} segmentation, deep learning models excel, whilst needing large amounts of labeled images.
    To deal with scarcity of labeled images, transfer learning is an\hl{ }approach 
    commonly used in computer vision, yet still underutilized in remote sensing. \hl{In the case of oil slicks,} differences between Sentinel-1 acquisition modes—such as the 
    interferometric wide (IW) in the North Sea and extra-wide (EW) in the Arctic—complicate direct model transfer.
    % and limit the potential for transfer learning %% Does this contradict the point of the paper?
    ...
    segmentation, even in challenging images with \hl{slick-like features}. Overall, \hl{few studies have addressed} transfer learning between SAR acquisition modes, and this work contributes to 
